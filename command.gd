...
func UpdateDefendSituation():
	StateDefensive = false
	MaxDefenders = int(get_tree().current_scene.GetEnemySoldiers().size() / 3)
	for enemy in enemy_list:
		var dir = my_flag_position - enemy.global_position
		if dir.length_squared() < 1000:
			StateDefensive = true
			MaxDefenders = get_tree().current_scene.GetEnemySoldiers().size()
			break
		pass
	pass
...
func CommandSoldiers():
	var defenders = 0
	for soldier in get_tree().current_scene.GetEnemySoldiers():
...
func _on_command_area_entered(area):
	...
	if area.id == "enemy":
		...
		add_child(impact) 
		impact.play()
	elif area.id == "blue_flag":
		
		flag_found = true
		flag_position = area.coordenate
		print("blue flag found")
		
		for item in get_parent().commander_list:
			
			item.flag_found = true
			item.flag_position = area.coordenate
			
			pass
	if life <= 0 :
		ship_explotion()
		destroy_sound()
		get_tree().current_scene.IALose(global_position)
