...
func ValidateDefensivePath(instructions: Array):
	var l = 0
	var r = 0
	var u = 0
	var d = 0
...
func _ready():
	...
	my_flag_position = get_tree().current_scene.GetRedFlagPosition()
...
func _on_enemy_soldier_area_entered(area):
	...
	if life <= 0 :
		ship_explotion()
		destroy_sound()
		var friends = get_tree().current_scene.GetEnemySoldiers()
		var i = 0
